...
(function () {
  ...
  function writeField(element, value) {
    ...
    var proto =
      element instanceof (unsafeWindow && unsafeWindow.HTMLTextAreaElement)
        ? unsafeWindow.HTMLTextAreaElement.prototype
        : element instanceof HTMLTextAreaElement
          ? HTMLTextAreaElement.prototype
          : element instanceof (unsafeWindow && unsafeWindow.HTMLInputElement)
    ...
    var effectiveSetter =
      ownDescriptor && ownDescriptor.set && ownDescriptor.set !== nativeSetter
        ? nativeSetter
        : nativeSetter;
  ...
  (async function () {
    ...
    // Auto-enable on Google KI Modus (exact URL: www.google.com/search?udm=50)
    if (!isActive && hostname === 'www.google.com' && location.pathname === '/search') {
      var params = new URLSearchParams(location.search);
      if (params.get('udm') === '50') {
        isActive = true; // session-only, not persisted to domains Set
      }
    }
