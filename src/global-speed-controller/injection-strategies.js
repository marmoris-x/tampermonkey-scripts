// src/global-speed-controller/injection-strategies.js — Three-tier fallback injection
//
 ...
 */
export function sendCmd(speed, enabled) {
  try {
    unsafeWindow.dispatchEvent(
      new unsafeWindow.CustomEvent(CONST.CMD_EVENT, { detail: { speed: speed, enabled: enabled } })
    );
...
export function setupUnsafeWindowFallback() {
  try {
    ...
    Object.defineProperty(uw.HTMLMediaElement.prototype, 'playbackRate', {
      configurable: true,
      enumerable:   true,
      get: function () { return state.enabled ? state.speed : fd.get.call(this); },
      set: function (rate) { if (fa || !state.enabled) fd.set.call(this, rate); }
    });
    ...
    unsafeWindow.__gsFallbackApply = function () {
      ...
      elements.forEach(function (el) {
        try {
          fa = true;
          fd.set.call(el, state.enabled ? state.speed : 1.0);
        } finally {
          fa = false;
        }
      });
  ...
  var id = setInterval(function () {
    ...
    try {
      ...
      for (var i = 0; i < els.length; i++) {
        var el = els[i];
        if (Math.abs(el.playbackRate - state.speed) > 0.001) {
