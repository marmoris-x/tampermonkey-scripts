 ...
 * Provides Shadow DOM-based UI components with full accessibility.
 * Uses adoptedStyleSheets with GM_addElement fallback for CSP bypass.
 * @module _ui
 */
...
/* --- Shared Styles (adoptedStyleSheets-ready) --- */
...
/**
 * Applies CSS to a ShadowRoot via adoptedStyleSheets with fallbacks.
 * Attempts CSSStyleSheet.replaceSync first, then GM_addElement,
 * then falls back to a plain <style> element.
 * @param {ShadowRoot} root - The shadow root to style
 * @param {string} cssText - CSS text content
 */
function applyStyles(root, cssText) {
  // Preferred: adoptedStyleSheets (avoids CSP issues with <style>)
  ...
  // Fallback: GM_addElement bypasses page CSP
  ...
  // Last resort: plain style element
...
export function createSidebar(opts = {}) {
  ...
  // Toggle tab (separate Shadow DOM element)
