...
(function () {
  ...
  async function scrollLoad() {
    window.__MPD__.triggerLazy(document);
    await sleep(150);
    ...
    function getUnloaded() {
      ...
      for (var i = 0; i < imgs.length; i++) {
        ...
        if ((!img.complete || !img.naturalWidth) &&
            (img.src && (img.src.startsWith('http') || window.__MPD__.extractLazySrc(img)))) {
          result.push(img);
        }
      }
      return result;
  ...
  window.__MPD__.scrollLoad = scrollLoad;
  window.__MPD__.navigateNext = navigateNext;
  window.__MPD__.waitForUrlChange = waitForUrlChange;
  window.__MPD__.guessNextUrl = guessNextUrl;
  window.__MPD__.clickNextPage = clickNextPage;
})();
