 ...
 */
export function callGeminiAPI(adsData, searchContext, topX, apiKey, modelKey, retryCount, onRetry, siteName) {
 ...
 */
export function fetchGeminiModels(apiKey, prefix, cachedSettings, saveAvailableModelsFn) {
 ...
 * @param {Object} cachedSettings - Current settings object
 */
export function showModelMapper(modelIds, prefix, cachedSettings) {
  var area = document.getElementById(prefix + '-model-area');
  if (!area) return;
  ...
  for (var ki = 0; ki < modelKeys.length; ki++) {
    ...
    for (var mi = 0; mi < modelIds.length; mi++) {
      ...
      html += '<option value="' + esc(modelIds[mi]) + '"' + selected + '>' + esc(modelIds[mi]) + '</option>';
    }
    html += '</select></div>';
  }
 ...
 * @param {Object} cachedSettings - Current settings object
 */
export function restoreModelSelect(prefix, cachedSettings) {
  var area = document.getElementById(prefix + '-model-area');
  if (!area) return;
  ...
  for (var ki = 0; ki < modelKeys.length; ki++) {
    ...
    html += '<option value="' + key + '"' + selected + '>' + m.icon + ' ' + m.label + ' - ' + esc(modelId) + '</option>';
  }
  html += '</select>';
