...
(function () {
  ...
  function renderSettingsView(prefix, settings, savedResults, siteName) {
    ...
    return [
      ...
      '<input type="password" id="' + prefix + '-api-key" placeholder="AIza..." value="' + escapeHTML(settings.apiKey) + '"',
  ...
  function renderResultsView(prefix, deals) {
    var items = deals.map(function (deal, index) {
      ...
      return [
        ...
        '<div style="font-weight:700;color:#333;font-size:14px;">' + medal + ' ' + escapeHTML(deal.title) + '</div>',
        ...
        (deal.description
          ? '<div style="font-size:11px;color:#555;line-height:1.4;margin-bottom:8px;max-height:60px;overflow:hidden;">' +
            escapeHTML(deal.description.substring(0, 150)) +
  ...
  function updateProgress(prefix, text, percentage, type, isWH) {
    ...
    if (!statusBar) {
      ...
      statusBar = TM.ui.createStatusBar({ accentColor: accent });
    }
   ...
   * @param {boolean} [isWH]
   */
  function showError(prefix, message, percentage, isWH) {
   ...
   * @param {boolean} [isWH]
   */
  function showSuccess(prefix, message, percentage, isWH) {
  ...
  /**
   * Returns HTML for the settings view and re-populates the modal.
   * @param {string} prefix
   * @param {Array} deals - Deals to display (or null for all saved)
   */
   ...
   * @param {string} prefix
   */
  function hideFloatingButton(prefix) {
   ...
   * @param {string} prefix
   */
  function showFloatingButton(prefix) {
    var btn = document.getElementById(prefix + '-dealfinder-btn');
   ...
   * @param {string} prefix
   * @param {Object} rankingEngineRef - Reference to window.__MDF__.rankingEngine
   */
  async function exportMarkdown(prefix, rankingEngineRef) {
    var rankEng = rankingEngineRef || (window.__MDF__ && window.__MDF__.rankingEngine);
    if (!rankEng) { TM.ui.createToast('Ranking engine not available!', { type: 'error' }); return; }

    var results = await rankEng.loadResults(prefix);
    if (!results) { TM.ui.createToast('Keine Results verfügbar!', { type: 'error' }); return; }
  ...
  window.__MDF__.uiPanel = {
    renderSettingsView: renderSettingsView,
    renderResultsView: renderResultsView,
    updateLiveRanking: updateLiveRanking,
    updateProgress: updateProgress,
    showError: showError,
    showWarning: showWarning,
    showSuccess: showSuccess,
    resetUI: resetUI,
    setUIRunningState: setUIRunningState,
    createModal: createModal,
    openModal: openModal,
    closeModal: closeModal,
    switchToResultsView: switchToResultsView,
    attachSettingsListeners: attachSettingsListeners,
    attachResultsListeners: attachResultsListeners,
    createDealFinderButton: createDealFinderButton,
