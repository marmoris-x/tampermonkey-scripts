...
(function () {
   ...
   */
  _CRE.getFilters = function getFilters($, sidebarRoot) {
   ...
   * @returns {boolean}
   */
  _CRE.passesFilter = function passesFilter(info, f) {
   ...
   */
  _CRE.compareCards = function compareCards(a, b, criterion) {
   ...
   */
  _CRE.applyFilterAndSort = function applyFilterAndSort(ctx) {
    if (ctx.cards.size === 0) return;
    ctx._observerPaused = true;

    var f = _CRE.getFilters(ctx._$, ctx.sidebar.root);
    ...
    var visible = all.filter(function (item) { return _CRE.passesFilter(item.info, f); });
    var hidden  = all.filter(function (item) { return !_CRE.passesFilter(item.info, f); });

    // Sort visible cards
    if (f.sort.length > 0) {
      visible.sort(function (a, b) {
        for (var ci = 0; ci < f.sort.length; ci++) {
          var r = _CRE.compareCards(a.info, b.info, f.sort[ci]);
          if (r !== 0) return r;
        }
    ...
    ctx._updateStats(visible.length, ctx.cards.size, _CRE.withData(ctx.cards));
