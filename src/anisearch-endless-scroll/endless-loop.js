...
(function () {
  ...
  async function runEndlessLoop(ratingMin, found, runId) {
    ...
    for (var i = 0; i < existingItems.length; i++) {
      if (!AES.passesRating(existingItems[i], ratingMin)) {
    ...
    while (nextUrl && currentPage <= MAX_PAGES) {
      ...
      for (var j = 0; j < newItems.length; j++) {
        if (AES.passesRating(newItems[j], ratingMin)) {
  ...
  window.__AES__.newRun = newRun;
  window.__AES__.isCurrentRun = isCurrentRun;
  window.__AES__.findContainer = findContainer;
  window.__AES__.findNextUrl = findNextUrl;
  window.__AES__.hideElements = hideElements;
  window.__AES__.hidePagination = hidePagination;
  window.__AES__.runEndlessLoop = runEndlessLoop;
  window.__AES__.CONTAINER_SELECTORS = CONTAINER_SELECTORS;
  window.__AES__.getItemSel = getItemSel;
})();
