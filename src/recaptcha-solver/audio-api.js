...
(function () {
  ...
  function getTextFromAudio(srcUrl, retry) {
    ...
    if (!retry) window.__RCS__.state.requestCount++;
    window.__RCS__.state.waitingStart = Date.now();
    log.log('Request to ' + server + ' | lang:' + lang + ' | attempt:' + window.__RCS__.state.requestCount +
      (retry ? ' [retry]' : ''));
      ...
      onload: function (response) {
        try {
          ...
          if (invalid) {
            ...
            window.__RCS__.state.waiting = false;
            return;
          }
          ...
          if (inAudioMode && audioSrcEl && audioSrcEl.src === srcUrl &&
            ...
            verifyBtn.click();
            window.__RCS__.state.submittedAt = Date.now();
  ...
  window.__RCS__.getLang = getLang;
  window.__RCS__.getBestServer = getBestServer;
  window.__RCS__.getTextFromAudio = getTextFromAudio;
})();
