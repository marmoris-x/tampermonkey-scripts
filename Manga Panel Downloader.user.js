...
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/manga-panel-downloader/image-finder.js
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/manga-panel-downloader/image-processor.js
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/manga-panel-downloader/page-navigator.js
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/manga-panel-downloader/ui-panel.js
...
(function () {
    ...
    var log = TM.createLogger('Manga Panel Downloader');
    ...
    function MangaDownloader() {
        ...
        // Build UI (Shadow DOM sidebar via TM.ui.createSidebar).
        // All element queries use this.ui.root.querySelector() — never document.getElementById().
        this.ui = window.__MPD__.buildUI(this.mangaMode);
        this._initUI();
    ...
    MangaDownloader.prototype._collectPageUrls = function () {
        ...
        var findAndSort = function () {
            var candidates = window.__MPD__.findImages(document);
            ...
            for (var i = 0; i < sorted.length; i++) {
                ...
                var srcs = window.__MPD__.allSrcsOf(c.el);
                var seen = false;
    ...
    MangaDownloader.prototype._scan = function () {
        ...
        window.__MPD__.setScanBtn(self.ui.scanBtn, true);
        self._setProgress(0);
        ...
        function producer() {
            ...
            function runProducer() {
                ...
                return self._collectPageUrls().then(function (found) {
                    ...
                    window.__MPD__.navigateNext();
                    return self._sleep(NAV_CLICK_WAIT_MS).then(function () {
                        return window.__MPD__.waitForUrlChange(prevUrl, NAV_TIMEOUT_MS).then(function (changed) {
                            if (!changed) return;
            ...
            function spawn(candidate) {
                var task = (function () {
                    return Promise.resolve().then(function () {
                        ...
                        return window.__MPD__.processImage(
        ...
        return phase.then(function () {
            ...
            self.scanning = false;
            window.__MPD__.setScanBtn(self.ui.scanBtn, false);
        });
    };
    ...
    MangaDownloader.prototype._renderResults = function () {
        ...
        window.__MPD__.addSegmentsToUI(this.segments, this.ui.resultsEl);
    };
    ...
    MangaDownloader.prototype._download = function () {
        ...
        var finish = function (zipBlob) {
            window.__MPD__.triggerDownload(zipBlob, name);
        ...
        window.__MPD__.buildZipBlob(checked).then(finish).catch(function (e) {
            ...
            var individual = function (idx) {
                ...
                window.__MPD__.triggerDownload(seg.blob, seg.filename);
