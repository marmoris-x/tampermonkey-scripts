...
`;
        ...
        for (const order of data.orders || []) {
          for (const item of order.items || []) {
            ...
            if (title && !IGNORE_LIST.some((bad) => title.includes(bad))) {
              ...
              const priceNum = cents > 0 ? cents / 100 : ((_a = order.total) == null ? void 0 : _a.amount) > 0 ? order.total.amount / 100 : 0;
              const priceFormatted = String(priceNum).replace(".", ",");
              const promo = (_b = order.promotions) == null ? void 0 : _b[0];
              const discountPct = promo && cents > 0 ? Math.round(Number(promo.amount) / cents * 1e3) / 10 : 0;
              games.push({
                orderId: order.orderId || "",
                title,
                price: priceFormatted,
                currency: item.currency || ((_c = order.total) == null ? void 0 : _c.currency) || "",
                acquiredDate: order.createdAtMillis ? new Date(order.createdAtMillis).toISOString().slice(0, 10) : "",
                marketplace: order.marketplaceName || "",
                quantity: item.quantity || 1,
                namespace: item.namespace || "",
                totalPaidCents: ((_d = order.total) == null ? void 0 : _d.amount) ?? cents,
                isPromotion: !!promo,
                savingsPrice: promo ? (promo.amount || 0) / 100 : 0,
                discountPct,
                taxCents: ((_e = order.tax) == null ? void 0 : _e.amount) ?? 0,
    ...
    async function exportToClipboard() {
      ...
      try {
        ...
        floater.done(msg);
        GM.notification({ title: "Epic Library Export", text: msg, timeout: 3e3 });
