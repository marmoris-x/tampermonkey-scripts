...
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/marketplace-deal-finder/ranking-engine.js
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/marketplace-deal-finder/api-gemini.js
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/marketplace-deal-finder/scraper-willhaben.js
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/marketplace-deal-finder/scraper-kleinanzeigen.js
// @require      https://raw.githubusercontent.com/marmoris-x/tampermonkey-scripts/main/src/marketplace-deal-finder/ui-panel.js
...
(function () {
  ...
  var Logger = TM.createLogger('Marketplace Deal Finder');

  /* ─── Module Shortcuts ─── */

  var MDF = window.__MDF__;
  var RE = MDF.rankingEngine;
  var UI = MDF.uiPanel;
  var API = MDF.geminiAPI;
  var SCRAPER = IS_WH ? MDF.wh : MDF.ka;
  ...
  async function waitIfPaused() {
    while (isPaused && !shouldStop) {
      await new Promise(function (r) { setTimeout(r, RE.PAUSE_POLL_INTERVAL); });
    }
  }
  ...
  function fetchFullDescription(url, retryCount) {
    ...
    var descSelectors = IS_WH ? MDF.wh.descSelectors() : MDF.ka.descSelectors();
    return new Promise(function (resolve) {
      GM_xmlhttpRequest({
        method: 'GET',
        url: url,
        timeout: RE.REQUEST_TIMEOUT,
        onload: function (response) {
          try {
            if (response.status >= 200 && response.status < 300) {
              ...
              if (fullDesc) {
                if (descriptionCache.size >= RE.MAX_CACHE_SIZE) {
                ...
                return;
              } else if (retryCount < RE.MAX_RETRIES && !shouldStop) {
            ...
            if (retryCount < RE.MAX_RETRIES && !shouldStop) {
              setTimeout(function () { fetchFullDescription(url, retryCount + 1).then(resolve); }, 1000);
              return;
            }
          }
  ...
  async function startDealFinder() {
    ...
    var model = modelEl ? modelEl.value : API.MODEL.FLASH;
    ...
    var currentSettings = await RE.loadSettings(P, cachedSettings);
    ...
    cachedSettings = RE.deepCopySettings(settings);
    ...
    UI.setUIRunningState(P);

    try {
      await processCurrentPage(apiKey, searchContext, topX, model, maxPages);
    } catch (error) {
      Logger.error('Error:', error);
      UI.updateProgress(P, 'Fehler: ' + error.message, 0, 'error', IS_WH);
  ...
  function pauseDealFinder() {
    ...
    UI.updateProgress(P, 'Pausiert - Klicke Fortsetzen...', 50, 'warning', IS_WH);
  }
  ...
  async function processCurrentPage(apiKey, searchContext, topX, model, maxPages) {
    ...
    UI.updateProgress(P, 'Seite ' + currentPage + ': Lade alle Anzeigen...', 10, 'info', IS_WH);
    window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
    await new Promise(function (r) { setTimeout(r, 1500); });
    window.scrollTo({ top: 0, behavior: 'smooth' });
    await new Promise(function (r) { setTimeout(r, 1500); });

    UI.updateProgress(P, 'Seite ' + currentPage + ': Sammle Anzeigen...', 15, 'info', IS_WH);
    ...
    if (!selectors) {
      ...
      if (pageText.indexOf('captcha') !== -1 || pageText.indexOf('challenge') !== -1) {
        ...
        pauseDealFinder();
        UI.updateProgress(P, 'CAPTCHA erkannt! Bitte lösen und Fortsetzen klicken', 50, 'warning', IS_WH);
        return;
      }
      throw new Error('Keine Anzeigen gefunden');
    }

    UI.updateProgress(P, 'Seite ' + currentPage + ': Sammle Basis-Daten...', 20, 'info', IS_WH);
    ...
    UI.updateProgress(P, 'Seite ' + currentPage + ': Lade Details (0/' + adsData.length + ')...', 30, 'info', IS_WH);
    var completedCount = 0;

    for (var bi = 0; bi < adsData.length; bi += RE.INITIAL_BATCH_SIZE) {
      await waitIfPaused();
      if (shouldStop) break;

      var batch = adsData.slice(bi, Math.min(bi + RE.INITIAL_BATCH_SIZE, adsData.length));
      var batchFns = [];
        ...
        (function (index, url) {
          ...
          batchFns.push(fetchPromise.then(function (result) {
            ...
            if (completedCount % 5 === 0 || completedCount === adsData.length) {
              UI.updateProgress(P, 'Seite ' + currentPage + ': Lade Details (' + completedCount + '/' + adsData.length + ')...',
      ...
      if (bi + RE.INITIAL_BATCH_SIZE < adsData.length) {
    ...
    var modelName = API.GEMINI_MODELS[model] ? API.GEMINI_MODELS[model].name : model;
    UI.updateProgress(P, 'Seite ' + currentPage + ': AI analysiert Angebote...', 75, 'info', IS_WH);
    ...
    var onRetry = function (status, retryNum, delaySeconds) {
      ...
      UI.showWarning(P, 'API ' + statusText + ' - Retry ' + retryNum + ' in ' + delaySeconds + 's...', 75, IS_WH);
    };

    var aiResult = null;
    try {
      aiResult = await API.callGeminiAPI(adsData, searchContext, topX, apiKey, model, 0, onRetry, SITE_NAME);
    } catch (error) {
    ...
    if (aiResult && aiResult.topDeals && aiResult.topDeals.length > 0) {
      ...
      UI.updateProgress(P, 'Seite ' + currentPage + ': ' + aiResult.topDeals.length + ' Top-Deals gefunden!', 90, 'success', IS_WH);
      UI.updateLiveRanking(P, allTopDeals, cachedSettings);
    }
  ...
  async function finishDealFinder() {
    UI.updateProgress(P, 'Erstelle finale Ranking-Liste...', 95, 'info', IS_WH);
    await RE.clearCrawlState(P);

    if (allTopDeals.length === 0) {
      UI.updateProgress(P, 'Keine Deals gefunden!', 100, 'error', IS_WH);
    ...
    if (shouldStop) {
      UI.updateProgress(P, 'Crawl gestoppt. Speichere bisherige Deals...', 100, 'warning', IS_WH);
      await RE.saveResults({ deals: allTopDeals, pages: currentPage, timestamp: new Date().toISOString() }, P);
      UI.switchToResultsView(P, allTopDeals);
      UI.resetUI(P);
      return;
    }

    // Deduplicate across pages
    allTopDeals = RE.deduplicateDeals(allTopDeals);

    // Global re-ranking across all collected deals
    if (allTopDeals.length > 1) {
      UI.updateProgress(P, 'Globales Re-Ranking aller Deals...', 97, 'info', IS_WH);
      var settings = cachedSettings || RE.DEFAULT_SETTINGS;
      allTopDeals = await RE.reRankGlobal(allTopDeals, settings.apiKey, settings.searchContext,
        settings.model || API.MODEL.FLASH, API, Logger);
    }

    await RE.saveResults({ deals: allTopDeals, pages: currentPage, timestamp: new Date().toISOString() }, P);
    UI.updateProgress(P, allTopDeals.length + ' Deals gespeichert!', 100, 'success', IS_WH);
  ...
  async function resumeCrawlIfActive() {
    ...
    var normalizedCurrentUrl = RE.normalizeUrl(crawlState.currentUrl);
    var normalizedWindowUrl = RE.normalizeUrl(window.location.href);
    var samePage = normalizedCurrentUrl && normalizedCurrentUrl === normalizedWindowUrl;
    var pageIncrement = samePage ? RE.SAME_PAGE_INCREMENT : RE.NEW_PAGE_INCREMENT;
  ...
  async function renderAndWireSettings() {
    var result = await RE.loadSettings(P, cachedSettings);
    cachedSettings = result.cachedSettings;
    var settings = result.settings;
    var savedResults = await RE.loadResults(P);

    var modal = document.getElementById(P + '-dealfinder-modal');
    if (!modal) return;
    modal.innerHTML = UI.renderSettingsView(P, settings, savedResults, SITE_NAME);

    // Wire settings view callbacks
    UI.attachSettingsListeners(P, {
      start: startDealFinder,
      pause: pauseDealFinder,
      stop: stopDealFinder,
      close: function () { UI.closeModal(P, isRunning); },
      showSavedResults: async function () {
        var sr = await RE.loadResults(P);
        if (sr) {
          UI.switchToResultsView(P, sr.deals);
          UI.attachResultsListeners(P, resultsCallbacks);
        }
      },
      apiKeyChange: async function (newKey) {
        var s = await RE.loadSettings(P, cachedSettings);
        cachedSettings = s.cachedSettings;
        var settingsObj = s.settings;
        if (settingsObj.apiKey !== newKey) {
          settingsObj.apiKey = newKey;
          await TM.storage.saveSetting(P + '_dealfinder_settings', JSON.stringify(settingsObj));
          cachedSettings = RE.deepCopySettings(settingsObj);
        }
      },
    ...
    API.restoreModelSelect(P, cachedSettings);
  }
    ...
    try {
      ...
      if (rawSettings) {
        try {
          ...
          var DEFAULT = RE.DEFAULT_SETTINGS;
          if (loaded.model && !{ flash: 1, pro: 1, nano: 1 }[loaded.model]) loaded.model = 'flash';
          cachedSettings = RE.deepCopySettings(Object.assign({}, DEFAULT, loaded));
        } catch (e) {
          cachedSettings = RE.deepCopySettings(RE.DEFAULT_SETTINGS);
        }
      } else {
        cachedSettings = RE.deepCopySettings(RE.DEFAULT_SETTINGS);
      }
      ...
      if (IS_WH) {
        ...
        if (!hasIndicator) {
          if (++initRetries >= RE.MAX_INIT_RETRIES) {
            Logger.warn('Max init retries reached - showing button anyway');
            UI.createModal(P);
            UI.createDealFinderButton(P, BTN_GRADIENT);
            UI.openModal(P);
